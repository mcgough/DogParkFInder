      ...
      .then(function() {
        db.parksusers
          ...
          .then(function(data) {
            ...
            async.each(
              parkids,
              function(parkid, callback) {
                db.park
                  .find({ where: { id: parkid } })
                  .then(function(park) {
                    var dogPark = {
                      name: park.name,
                      id: park.id,
                    ...
                    if (_.findWhere(times, { parkid: park.id })) {
                      dogPark.checkInCount = _.where(times, {
                        parkid: park.id
                      }).length;
                    } else {
...
router.post("/remove", function(req, res) {
  ...
  var id = parseInt(req.body.userid);
  db.parksusers
    .destroy({ where: { parkId: park, userId: id } })
    .then(function() {
...
router.get("/:id", function(req, res) {
  ...
  if (id === "undefined") {
    ...
    db.park
      ...
      .then(function() {
        db.checkin
          .findAll({ where: { parkId: parkid } })
          .then(function(parkCheckins) {
