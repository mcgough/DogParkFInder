...
router.get('/:id',function(req,res){
  var user = req.getUser(),
      near = req.query.q,
      url = 'https://api.foursquare.com/v2/venues/search?client_id=' + fourSq + '&client_secret=' + fourSqSec + '&v=20130815&near=' + near + '&radius=6000&query=dog park';
  request(url,function(error,response,data) {
    if(!error && response.statusCode === 200){
      var locations = JSON.parse(data),
          venues = locations.response,
          favorites = [],
          times = [],
          parks = [];
