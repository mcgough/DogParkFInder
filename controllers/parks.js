...
function withInHourCheck(time) {
  return (
    (new Date().getTime() / 1000 / 3600) * 60 -
      (Date.parse(time) / 1000 / 3600) * 60 <=
    60
  );
}
...
router.get("/nearme", function(req, res) {
  ...
  url =
    ...
    "," +
    coords.lng +
    "&radius=2500&query=offleash dog park";
  request(url, function(err, response, data) {
    if (!err && response.statusCode === 200) {
      ...
      venues.forEach(function(venue) {
        venue.location.distance = parseFloat(
          metresToMiles(venue.location.distance)
        ).toFixed(1);
      });
      ...
      if (!user) {
        ...
        var userID = user.id,
          ...
          times = [],
          userHerePark;
        db.checkin.findAll().then(function(response) {
          ...
          db.park.findAll().then(function(parks) {
            db.parksusers
              ...
              .then(function(ids) {
                ...
                venues.forEach(function(park) {
                  ...
                  if (_.findWhere(checkedInDogParks, { name: park.name })) {
                    park.checkInCount = _.findWhere(checkedInDogParks, {
                      name: park.name
                    }).count;
                  } else {
                    park.checkInCount = 0;
              ...
              .then(function(ids) {
                ...
                venues.forEach(function(park) {
                  ...
                  if (_.findWhere(checkedInDogParks, { name: park.name })) {
                    ...
                    park.appParkId =
                      _.findWhere(checkedInDogParks, { name: park.name })
                        .parkId || false;
                  } else {
                    park.checkInCount = 0;
                  }
